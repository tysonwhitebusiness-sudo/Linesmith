 ...
 *
 * U5: the duplicate `Skeleton` primitive that used to live here is gone — the
 * one in `components/ui/` is the only one now, and these compose it. What stays
 * here is the SHAPES: a skeleton sized like the component it stands in for,
 * which is page knowledge, not kit knowledge.
 *
    ...
    <article className="lb-card p-3.5" aria-hidden>
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 flex-1 items-start gap-2.5">
          <Skeleton w={36} h={36} round="rounded-full" />
          <div className="min-w-0 flex-1 space-y-1.5 pt-0.5">
            <Skeleton w="55%" h={13} />
            <Skeleton w="70%" h={11} />
          </div>
        </div>
        <Skeleton w={74} h={20} round="rounded-full" />
      </div>
      ...
      <div className="mt-2.5 flex gap-1.5">
        {[0, 1, 2, 3, 4].map((i) => (
          <Skeleton key={i} w={28} h={28} round="rounded-full" />
        ))}
      </div>

      <div className="mt-3 flex gap-2">
        <Skeleton w="100%" h={36} round="rounded-lg" className="flex-1" />
        <Skeleton w={44} h={36} round="rounded-lg" />
      </div>
    </article>
  );
}
  ...
  return (
    <div className="space-y-3" role="status" aria-label="Loading player">
      <section className="lb-card flex items-center gap-3 p-4">
        <Skeleton w={56} h={56} round="rounded-full" />
      ...
      <section className="lb-card space-y-3 p-3">
        ...
        <div className="flex gap-2">
          {Array.from({ length: 5 }, (_, i) => (
            <Skeleton key={i} w={54} h={52} round="rounded-lg" />
          ))}
        </div>
        <Skeleton w="100%" h={44} round="rounded-lg" />
      </section>
      <span className="sr-only">Loading…</span>
    </div>
  );
