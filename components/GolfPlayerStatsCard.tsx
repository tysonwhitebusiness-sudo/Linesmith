...
import { SegmentedToggle, Tooltip } from './ui';
...
function GolfStatRangeRow({
  ...
  return (
    <div className="py-1">
      <div className="flex items-center justify-between text-overline font-normal uppercase tracking-wide text-ink-muted">
        <span className="truncate">{stat.label}</span>
        <Tooltip content={`${stat.rank} of ${stat.poolSize}`}><span className="shrink-0 tabular-nums">
      ...
      <div className="relative mx-2.5 h-3">
        {avgPct != null ? (
          <span
            className="absolute -translate-x-1/2 whitespace-nowrap text-overline tracking-normal font-medium text-ink-muted"
            style={{ left: `${avgPct}%` }}
          >
      ...
      <div className="relative mx-2.5 h-4">
        ...
        {avgPct != null ? (
          <Tooltip content={`Tour average: ${stat.avgValue!.toFixed(stat.decimals)}`}><span
            className="absolute top-1/2 h-3 w-[2px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-ink-faint"
            style={{ left: `${avgPct}%` }}
          /></Tooltip>
        ) : null}
        <Tooltip content={`${name}: ${stat.value.toFixed(stat.decimals)}`}><div
      ...
      <div className="relative mx-2.5 h-3">
        <span
          className="absolute -translate-x-1/2 whitespace-nowrap text-overline tracking-normal font-semibold tabular-nums text-ink"
          style={{ left: `${playerPct}%` }}
        >
      ...
      <div className="mx-2.5 mt-0.5 flex items-center justify-between text-overline font-normal tracking-normal tabular-nums text-ink-muted">
...
export function GolfPlayerStatsCard({
  ...
  return (
    <div className="lb-card lb-card-interactive space-y-3 p-3">
      <div>
        <h3 className="mb-2 text-overline font-semibold uppercase tracking-wide text-ink-muted">
        ...
        <p className="mt-2 text-overline font-normal tracking-normal text-ink-muted">Source: pgatour.com official stats, not a documented API — see /diagnostics for freshness.</p>
      </div>

      <div className="border-t border-line-soft pt-3">
        <h3 className="mb-2 text-overline font-semibold uppercase tracking-wide text-ink-muted">Recent tournaments</h3>
        {recentEvents.length === 0 ? (
          <p className="text-label font-normal text-ink-muted">No tournament history found for this golfer this season.</p>
        ) : (
          <ul className="divide-y divide-line">
            {recentEvents.map((e) => (
              <li key={e.eventId} className="flex items-center justify-between py-1.5 text-label font-normal">
