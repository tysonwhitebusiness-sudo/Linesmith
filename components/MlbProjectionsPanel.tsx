...
/**
 * Phase 4.10 — MLB's call site for the shared `StatsBoard`.
 *
...
import type { StatsBoardData } from '@/lib/sports/mlb/adapters/statsBoardAdapter';

type Payload = StatsBoardData & { unnamedOmitted: number };

export default function MlbProjectionsPanel() {
  const [data, setData] = useState<Payload | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let live = true;
    fetch('/api/mlb/projections')
      .then((r) => {
  ...
  if (error) {
    return (
      <section className="lb-card mx-auto max-w-3xl p-5">
        <h2 className="text-lg font-semibold tracking-tight">MLB projections</h2>
