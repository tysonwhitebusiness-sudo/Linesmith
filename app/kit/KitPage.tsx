    ...
    <main className="mx-auto max-w-[1100px] px-4 py-8">
      <header>
        ...
        <p className="mt-2 max-w-[70ch] text-body-sm text-ink-muted">
          ...
          tables, U3 the field family, U4 the overlays, U5 the borrowed pieces, U6 the two table additions below, C0 Electric Turf and the team-colour pieces.
        </p>
      </header>
      ...
      >
        <Row name="bg-good/10" note="tinted chip fill — alpha now compiles to color-mix()">
          <span className="rounded-ctl bg-good/10 px-3 py-1.5 text-body-sm text-good-ink">good at 10%</span>
          <span className="rounded-ctl bg-bad/10 px-3 py-1.5 text-body-sm text-bad-ink">bad at 10%</span>
          <span className="rounded-ctl bg-warn/10 px-3 py-1.5 text-body-sm text-warn-ink">warn at 10%</span>
        </Row>
      ...
      >
        ...
        <Row name="Team bands" note="bandColors(): white text at 4.5:1, or charcoal">
          {TEAM_COLOR_SAMPLES.map(({ name, color: c }) => {
        ...
        <Row name="Avatar ring · AvatarGroup tooltip" note="the hero headshot; stacked book marks">
          <span className="inline-flex h-20 items-center rounded-card px-4" style={{ backgroundImage: bandGradient(bandColors(VIKINGS)) }}>
