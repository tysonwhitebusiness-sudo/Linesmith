...
/** `/soccer/[league]/game/[gameId]` — the soccer equivalent of `/nfl/game/[gameId]`. */
export default function SoccerGameDetailPage() {
  ...
  const onSelectCandidate = (subjectId: string | null, dimension?: string) => {
    ...
    router.replace(`/soccer/${league}/game/${gameId}${suffix ? `?${suffix}` : ''}`);
  };
      ...
      <header className="sticky top-0 z-20 border-b border-line bg-paper/95 backdrop-blur">
        <TopBar
          ...
          leading={
            <button
              type="button"
              onClick={() => router.push(`/soccer/${league}`)}
