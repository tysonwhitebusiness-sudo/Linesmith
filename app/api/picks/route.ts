...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ picks: await listPicks(sport, user.id) });
}
...
export async function PATCH(request: Request) {
  ...
  const pick = await updatePickOdds(body.id, odds, body.source ?? 'manual', user.id);
...
export async function DELETE(request: Request) {
  ...
  if (all !== null) {
    await clearPicks(params.get('sport') ?? undefined, user.id);
    return NextResponse.json({ ok: true });
  }
  if (!id) return NextResponse.json({ error: 'id is required' }, { status: 400 });

  await deletePick(Number(id), user.id);
  return NextResponse.json({ ok: true });
}
