...
const MARKET_KEY_ALIASES: Record<string, MarketKey> = {
  ...
  // The new canonical keys (python-odds-service/src/scraper_markets.py NEW_KEYS),
  // as identity aliases so labels and the drift test know them, and comparenbet's
  // SportsGameOdds spellings, which the paid feeds share. Kept in lockstep with
  // entity_resolution.py (tests/config-drift.test.ts).
...
const BOOKMAKER_ALIASES: Record<string, string> = {
  ...
  // Kept in lockstep with entity_resolution.py (tests/config-drift.test.ts).
