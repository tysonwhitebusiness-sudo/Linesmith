...
test('every Python ranking is mirrored, with the same title, promo and not-held note', () => {
  const py = pyRankings();
  assert.equal(py.length, 48, "the parser found every RankingDef (8 specials + 40 spotlights; P12 added the four odds flags)");
...
test('F0: every ranking is the same KIND in both languages', () => {
  const py = pyRankings();
  assert.equal(py.filter((r) => r.kind === 'spotlight').length, 40, 'the Python file declares 40 spotlights');
  assert.equal(Object.keys(SPOTLIGHT_RANKINGS).length, 40, 'the registry mirrors 40 spotlights');
