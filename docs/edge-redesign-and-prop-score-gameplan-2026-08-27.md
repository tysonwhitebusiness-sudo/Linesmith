...
**Phase 3 — per-sport prop-score build, now starting from the corrected edge. Real infrastructure built and tested tonight.** `predict/generic_player_gamelog.py` (real ESPN gamelog fetcher, confirmed live for NFL and NBA) + `predict/generic_prop_score.py` (ties real gamelog data through the same, unchanged `edge_model.py`/`good_bets.py`/`live_edge.py`/`prop_score.py` chain MLB uses). Real end-to-end test against Jayson Tatum's actual 2025-26 NBA season: model probability 71.3% for points-over-17.5 (his real season rate is 78%), score 83/A — sane. Two real bugs found and fixed live during this test: (1) an unfiltered full-roster league rate diluted a real star's own strong rate by averaging in low-minute bench players; (2) ESPN's gamelog endpoint silently returns a partial season without an explicit `season` parameter (16 games instead of a real 78). Not yet wired: the X (matchup-favorability) signal — the uncommitted TS `teamDefenseAllowed.ts` groundwork (NBA/NHL/CFB) isn't ported to this Python path, so `matchup_favorable` is always `None` today, honestly, not silently. NFL/CFB/Soccer/Tennis dimension configs not yet built — the infrastructure is sport-generic (config-driven, matching `generic_team_elo.py`'s own pattern), but each sport still needs its own real `DimensionConfig` list and its own coverage check (Phase 4 below).

**Phase 4 — repeat this exact coverage audit per sport, not assumed. Done, real result: zero.** Checked live for NFL/CFB/NBA/NHL/Soccer against real `game_picks`-known games: **0 sharp-tier (Pinnacle/Circa/Novig/Kalshi) rows for any of them right now**, and even total prop coverage of any kind is thin (3 NFL games, 3 CFB games, 0 for NBA/NHL/Soccer). This is not a bug — it's the real, honest current state, and the same mechanism already proven for MLB (73,910 real candidate-sides tested, Tier 1 2.2%/Tier 2 18.4%/Tier 3 79.4%) will work identically for these sports the moment real prop coverage lands for them; right now E will mostly return `None` (honest absence, weight redistributed to M/P/X) for every non-MLB sport rather than a fabricated signal. Worth re-checking once each sport's real season is underway and its own prop pipeline has been running for a while, not assumed fixed by anything built tonight.
