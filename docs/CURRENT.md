...
**R7 SIGNED OFF 2026-09-16; R8 (game page) STARTED — MLB first (operator). R8.1 in progress: 8.1a (shared shell + MLB final recap, any pk resolves) and 8.1b (before-start research: strength, form, head to head, starters, prop history, injuries; kept below a final) committed; next live (8.1c, verify on live games before 2026-09-27), then stop for MLB sign-off. Record in the plan's R8 section.** R7: every team sport is on `TeamResearchPage`; `TeamDetail.tsx` and its hooks, adapters and four routes are deleted.** R7-F1 routed to the model track: MLS 2026 game logs start 2026-08-15, so MLS team stats and rosters cover 4-5 of ~25 games (the page says so). Also fixed: the NHL team-id map sent Utah to its old id (59, not 68). R7.1: `/api/team-research` +
