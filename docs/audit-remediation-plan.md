...
| Q39 | **The fitted Platt calibrations are applied on the MLB prop path.** 4.3 fitted seven and nothing consumed them; the one serve-time reader asks for a market none of them cover. Changes what a user sees: Platt with a<1 compresses confidence (0.700 -> 0.606), so fewer picks clear the 3% edge bar. That is the calibration working. | Phase 4 gate |
| Q40 | **Q37 is REVERSED: no re-fit. The MLB total mismatch is accepted and the TRAINING side is aligned to serving instead.** Q37 was decided on my statement that the gap was "up to 11 points" — that was the shift on an INPUT to the regression, not on the output. Measured against v8's real weights (rawOverProb −0.373 and simOverProb +1.040 against marketProbCentered +3.737, so the model is driven by the market price and these are small corrections), the shift on the final probability is **at most 1.03 percentage points**, typically 0.3–0.7. A multi-hour re-fit to recover a point is not worth it. Re-fitting would also have made things WORSE: 4.11 changed the sim feature in training but not at `odds_lines_cycle.py:491`, so new weights would have mismatched the 1.040 feature — roughly triple today's error on the 0.373 one. `model_fit.py`'s sim feature is reverted to Poisson to match serving, so whenever someone does re-fit, the weights match what is served. To go fully negative binomial later: change both call sites AND re-fit, together. | Phase 4 gate |
