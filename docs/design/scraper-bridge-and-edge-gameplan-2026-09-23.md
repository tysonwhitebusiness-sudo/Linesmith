...
| D24 | **Bridge storage policy (2026-09-25, P4): everything, compact, 10 days hot, disk held at 27 GB.** What reaches Supabase: every matched, de-flapped price change from all sources, every book and every relay copy, pre-game and in-game (8.1M/day measured 2026-09-24), in a compact history format (117 B/row measured, vs 345 B for `prop_odds_history` today); the existing `prop_odds_history` is converted to the same format in P5. Hot window: 10 days in Postgres; older rows move to the Parquet corpus in Storage (never deleted); the laptop keeps everything. What stays on the laptop only: flaps, non-price rows (openers go to `market_openers`), and unmatched rows beyond `scraper_unmatched_prices`. Disk: stays at the provisioned 27 GB (~6.1 GB used on 2026-09-25); a disk guard in the Python worker keeps database + WAL under 85% of it so Supabase never autoscales (next step 40.5 GB); near the limit it moves the oldest history to the corpus sooner and alerts, the bill never moves. No corpus reader in the app until a page needs more than 10 days. Measurements: `docs/design/odds-build/results/P4-storage-audit.md`. |
| D25 | **Cost ceiling (2026-09-25): $50/month all-in** = Supabase Pro $25 + at most $25 more, across Supabase, Render and (later) Vercel, **enforced by caps in our own code**, not by the providers' spend caps. Known extra today: Render ~$10 (one $7 service + ~$3 usual overage), Supabase disk ~$2.38 (27 GB). Before anything else that can raise a bill ships: an audit pulling every provider's live price and usage into one monthly total, then meters, alerts and automatic brakes ahead of the limit. |
