...
| **U4** | **Overlays.** `Modal` (400 · 560 · 720, bottom sheet below 768), `SlideoutMenu`, `Dropdown` (with an optional static header), `Popover` — all React Aria. `DrillDownPanel` is the `SlideoutMenu` with its props kept. `SlipModal` (its scrim button and mount/animation state machine deleted), `AccountMenu` and the diagnostics resume dialog moved. Enter/exit keyframes in `globals.css`; reduced motion fades (`.lb-overlay-move`). | **DONE 2026-09-21.** Measured in a fresh tab on prod: the slip traps Tab, closes on Escape and on the scrim, returns focus to the Slip button, and is a bottom sheet at 400. The Dropdown opens and acts from the keyboard. | `tests/ui-overlays.test.ts`: no `createPortal` / `role="dialog"` / `aria-modal` and no `fixed inset-0` scrim outside the kit; DrillDownPanel is the SlideoutMenu with the same props; the scrim is ink/40 with no blur; the moving overlays fade under reduced motion. `ui-buttons` lost the slip scrim and one diagnostics button | **done** |
...
| **U6** | **Page sweep.** Every hand-typed `text-[Npx]` snapped to the ramp (never below 11px; `font-normal`/`tracking-normal` added where the token's own weight would have restyled it — `scripts/u6_sizes.py`); every native `title=` → the kit `Tooltip` (`scripts/u6_titles_to_tooltip.py`), which now passes children through when its content is empty; hex → `white`, the `masters` token, or the chart palette. Golf's leaderboard and three hole grids, the pitch table, the golf scorecard and diagnostics' eleven tables are `DataTable` (which gained `columnGroups`, U-7, and `ink`, U-15); diagnostics' 13 buttons and 16 chips are the kit. | **DONE 2026-09-21.** Swept `/mlb`, `/golf`, `/golf/schedule`, `/tennis/atp/schedule`, a player, a team, a game, `/nfl/teams`, `/login`, `/privacy` at 1440 and 400 in fresh tabs: no page errors, document width = viewport, no rendered text under 11px outside the frozen Scan board. `/diagnostics` redirects signed-out, so it is covered by tests only (Q18). | `tests/ui-sweep.test.ts`: sizes, native titles and hex at zero in scope with named exceptions; golf colours par with `ink`, never `tone`. `ui-tables`, `ui-buttons` and `ui-pieces` lost every U6 entry | **done** |
| **U7** | **Close.** The allowlists are gone: every U1–U6 ratchet reached zero in scope. The old glider `SegmentedToggle.tsx` joined `OUT_OF_SCOPE` (its last importer is Scan's `FilterBar`) and was restored to its pre-U6 bytes; `global-error.tsx` stays exempt from the Button rule for a reason that does not expire. `/kit` gained the table additions. `CLAUDE.md` gained "UI primitives — one component per job". | **DONE 2026-09-21.** 606 tests pass; the pitch table and `/kit` rendered at 1440. | none; this is the audit | **done** |
...
| U-13 | U4 | **The slip had no focus trap and no Escape; the account menu had no Escape and caught clicks with an invisible full-screen div.** Only `DrillDownPanel` had a real trap, hand-written. All four are React Aria's now, so the behaviour is one implementation | fixed in U4 | **fixed** |
| U-14 | U4 | **A worn tab makes an overlay look stuck.** In the long-lived verification tab, React Aria's exit waited ~3.6s because the tab's animation timeline had stopped advancing (currentTime 0 while "running"); a fresh tab removed the same slip in 537ms. The scrim now takes no clicks while exiting, so even a lagging removal cannot block the page. Same lesson as the fresh-tab note: verify overlays in a fresh tab | noted, mitigated | **fixed** |
| U-15 | U6 | **`tone` is a RESULT, not a colour.** Converting golf's par-coloured cells to `tone` put a "W" chip beside every under-par round — `tone` draws the W/L chip of a results column. Caught on render. `Column.ink` is the categorical colour on the value itself (under par good, over par bad); `tone` stays for results. Pinned in `ui-sweep` | fixed in U6 | **fixed** |
| U-16 | U6 | **Scan's cell components are part of the freeze.** `StatCells` and `OddsChip` draw the Scan table's cells; converting their native titles to the Tooltip made every Scan cell a tab stop. Reverted and named as exceptions in `ui-sweep` rather than widening `OUT_OF_SCOPE` | noted | **fixed** |
