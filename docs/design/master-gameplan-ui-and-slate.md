...
| 10 | S2 Movers + price gaps | S | S1 | yes | no | **DONE** — Price outliers and Line disagreements 2026-09-20; **Movers 2026-09-21 (MV1–4)** once SL-29 showed the noise was live pricing |
...
| SL-29 | after S6 | **SL-18 re-measured: Movers' noise was mostly LIVE pricing, not bad books.** The history tables keep logging after first pitch and the S2 readers never cut there. MLB, 36h: pre-game props move 1.07 pts on average with 0.3% of changes over 10 pts; live, 5.58 pts and 18.1%. Game lines: 2.2% vs 29.9%. What's left pre-game is FLICKER (Fanatics reverts 43–51% of its changes), which a net first→latest move cancels. PrizePicks is off the cross-book median on 88% of quotes. Plan: `docs/design/movers-and-spotlights-gameplan.md` | Movers is buildable after an MV0 re-measure for NFL/soccer | open |
| SL-30 | MV1–4 | **Movers built.** Four things were wrong on first render and are pinned: the main line is the one priced nearest EVEN (books post alternate lines first, so "each book's first line" read Davante Adams as 1.5 → 5.5); the trend window ends at now, not at the start; Split and "line first → now" only describe a row shown at the main line; the nav count is the rows the card lists. See `docs/design/movers-and-spotlights-gameplan.md` | fixed in MV1–4 | **fixed** |
